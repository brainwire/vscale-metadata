...
GOXGOPATH := $(PWD)/hack/gox
GOX := $(GOXGOPATH)/bin/gox
...
.PHONY: build-all
build-all: ## Build rest server docker image
	@cd $(WORKSPACE)/ &&\
	$(GOX) -output=$(DISTFOLDER)/"{{.OS}}_{{.Arch}}" -os="linux"
...
get-tools: ## Download all tools dependencies in hack sub-directory
	...
	@mkdir -p $(GO2XUNITGOPATH) && cd $(GO2XUNITGOPATH) && GOPATH=$(GO2XUNITGOPATH) go get github.com/tebeka/go2xunit
	...
	@mkdir -p $(GOXGOPATH) && cd $(GOXGOPATH) && GOPATH=$(GOXGOPATH) go get github.com/mitchellh/gox
...
$(GO2XUNIT):
	@cd $(GO2XUNITGOPATH) && GOPATH=$(GO2XUNITGOPATH) go install github.com/tebeka/go2xunit
...
$(GOX):
	@cd $(GOXGOPATH) && GOPATH=$(GOXGOPATH) go install github.com/mitchellh/gox
